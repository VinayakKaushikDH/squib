...
struct StateEngineTests {
    ...
    @Test("reset() clears sessions and returns to idle")
    func resetClearsState() {
        ...
        engine.reset()
        #expect(engine.currentState == .idle)
        #expect(engine.sessionSnapshot.isEmpty)
    }

    @Test("reset() fires onStateChange when state was not idle")
    func resetFiresStateChange() {
        let engine = StateEngine()
        engine.handle(HookEvent(hookEventName: "UserPromptSubmit", sessionId: "s1", toolName: nil))
        var received: [PetState] = []
        engine.onStateChange = { received.append($0) }
    ...
    @Test("reset() fires onSessionsChange with empty snapshot")
    func resetFiresSessionsChange() {
        let engine = StateEngine()
        engine.handle(HookEvent(hookEventName: "PreToolUse", sessionId: "s1", toolName: nil))
        var lastSnapshot: [String: PetState] = ["placeholder": .idle]
        engine.onSessionsChange = { lastSnapshot = $0 }
        engine.reset()
        #expect(lastSnapshot.isEmpty)
    }

    // MARK: - sessionSnapshot

    @Test("sessionSnapshot reflects active sessions")
    func sessionSnapshotAccurate() {
        let engine = StateEngine()
        engine.handle(HookEvent(hookEventName: "UserPromptSubmit", sessionId: "abc", toolName: nil))
        engine.handle(HookEvent(hookEventName: "PreToolUse",       sessionId: "xyz", toolName: nil))
        let snap = engine.sessionSnapshot
        #expect(snap["abc"] == .thinking)
        #expect(snap["xyz"] == .working)
    }

    @Test("sessionSnapshot is empty on a fresh engine")
    func sessionSnapshotEmptyInitially() {
        let engine = StateEngine()
        #expect(engine.sessionSnapshot.isEmpty)
    }
}
